...
def build_master(
    ...
    base = (
        ...
        .agg(
            ...
            class_value_original=("class_value_original", first_value),
            class_name_original=("class_name_original", first_value),
    ...
    master["detalle_tipo_fuente_dominante"] = str(
        source_cfg.get("fuente_reporte", "nueva_fuente_a3")
    )
    ...
    # La nueva fuente no trae conf_integrada observada. Se conserva el esquema A1 sin NULL.
    ...
    report_md.write_text(
        "\n".join(
            [
                ...
                f"- Capa: `{out_layer}`",
                "",
                "La salida replica el esquema de columnas de A1 y agrega únicamente columnas originales Panamá para trazabilidad temática.",
                "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scoring A1 para caso Panamá v2 con esquema compatible A1.")
...
def main() -> None:
    ...
    score_cfg = config["quality_scoring_a1"]
    print("============================================================")
    print("PGBM - Scoring A1 compatible - caso Panamá v2")
    print("============================================================")
    master = build_master(
        data["records"],
        data["xy_groups"],
    ...
    cols = a1_columns() + PANAMA_EXTRA_COLUMNS + ["geometry"]
